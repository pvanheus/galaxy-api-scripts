...
def get( api_key, url ):
    ...
    try:
        reply = urllib.request.urlopen( url ).read().decode('utf-8')
        return json.loads( reply )
    except ValueError as e:
        print(("URL did not return JSON data: {}".format(e)))
        sys.exit(1)
...
def post( api_key, url, data ):
    ...
    """
    url = make_url( api_key, url )
    req = urllib.request.Request( url, headers = { 'Content-Type': 'application/json' }, data = json.dumps( data ) )
